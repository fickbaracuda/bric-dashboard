"""
scripts/run_payment_agent_produk_migration_remote.py

Jalankan migration Payment Agent Produk di server PRODUCTION dengan aman
lewat SSH — mengikuti pola persis run_dm_control_tower_migration_remote.py.
...
APA YANG DILAKUKAN SCRIPT INI:
  ...
  2. Mengecek dulu file migration & runner-nya ADA di server:
       backend/src/migrations/create_payment_agent_produk.sql
       backend/scripts/run-payment-agent-produk-migration.js
     Kalau salah satu tidak ada -> BERHENTI, tidak menjalankan apa pun.
  3. Menjalankan HANYA migration Payment Agent Produk:
       node backend/scripts/run-payment-agent-produk-migration.js
     TIDAK menjalankan migration lain apa pun.
  4. Setelah selesai, memverifikasi 4 tabel berikut benar-benar ada di
     database (query read-only lewat psql, DATABASE_URL TIDAK PERNAH
     ditampilkan di layar/log):
       payment_agent_produk_metrics, payment_agent_produk_deviation,
       payment_agent_produk_sync_log, payment_agent_produk_config
  5. TIDAK insert data apa pun, TIDAK menjalankan sync apa pun.

Cara pakai:
  python scripts/run_payment_agent_produk_migration_remote.py
"""
...
MIGRATION_SQL_REL = "backend/src/migrations/create_payment_agent_produk.sql"
MIGRATION_RUNNER_REL = "backend/scripts/run-payment-agent-produk-migration.js"
EXPECTED_TABLES = [
...
def run_migration(config: dict) -> bool:
    ...
    try:
        ...
        verify_cmd = (
            ...
            "SELECT string_agg(tablename, ',') FROM pg_tables WHERE tablename LIKE 'payment_agent_produk_%'"
            '"'
        )
        ...
        print(f"   Tabel payment_agent_produk_* ditemukan di database: {found_tables if found_tables else '(tidak ada)'}")
        if missing:
            print(f"[PERINGATAN] Tabel berikut BELUM ditemukan: {missing}")
            return False
        print("[OK] Semua 4 tabel Payment Agent Produk terkonfirmasi ada.")

        print("\n>>> [4/4] Migration Payment Agent Produk selesai dengan sukses.")
        return True
    finally:
        client.close()
...
def main():
    print("==============================================================")
    print("  BRIC DASHBOARD — MIGRATION PAYMENT AGENT PRODUK (production)")
    print("==============================================================")
    print("Script ini HANYA menjalankan migration Payment Agent Produk.")
    ...
    print()
    print("=== Migration Payment Agent Produk SELESAI (sukses). ===" if success
          else "=== Migration Payment Agent Produk GAGAL / BELUM LENGKAP — lihat pesan di atas. ===")
