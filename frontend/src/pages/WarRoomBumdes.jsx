...
const TIPE_COLORS = ['#0D9488','#0EA5E9','#9333EA','#F59E0B','#EF4444','#EC4899','#8B5CF6','#14B8A6'];
...
function buildInsight(s, bulan) {
  ...
  const paragraph = [
    `Total ${fmtN(s.total_outlet)} outlet BUMDes terdaftar, ${fmtN(s.mat)} aktif bertransaksi di ${curr} (activation rate ${matPct}%).`,
    arahTrx && arahRev
    ...
    newReg > 0 ? `${fmtN(newReg)} outlet BUMDes baru registrasi di ${curr}.` : '',
  ...
  if (churned>0)    recs.push({ icon:'🚨', title:'Recovery Call Outlet Hilang',          priority:'Prioritas Tinggi', color:'#DC2626',
    text:`${fmtN(churned)} outlet BUMDes aktif di ${prev} tapi tidak ada transaksi di ${curr}. Hubungi pengelola segera agar bisa diaktifkan kembali.` });
  if (declining>0)  recs.push({ icon:'📉', title:'Retensi Outlet Declining',              priority:'Prioritas Tinggi', color:'#EF4444',
    text:`${fmtN(declining)} outlet BUMDes mengalami penurunan TRX vs ${prev}. Follow-up langsung, identifikasi kendala operasional.` });
  ...
  if (nmat>0)       recs.push({ icon:'🌟', title:'Onboarding Outlet BUMDes Baru',         priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(nmat)} outlet BUMDes baru aktif di ${curr}. Lakukan pendampingan terstruktur agar konsisten aktif bulan berikutnya.` });
  if (belumAktif>0) recs.push({ icon:'⚠️', title:'Aktivasi Outlet Belum Bertransaksi',   priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(belumAktif)} outlet BUMDes terdaftar tapi belum pernah bertransaksi. Kunjungi pengelola dan fasilitasi transaksi pertama.` });
  if (growing>0)    recs.push({ icon:'📈', title:'Pertahankan Momentum Growth',           priority:'Prioritas Rendah', color:'#10B981',
    text:`${fmtN(growing)} outlet BUMDes sedang tumbuh. Berikan apresiasi dan dorong peningkatan volume layanan transaksi.` });
  if (stable>0)     recs.push({ icon:'📌', title:'Dorong Outlet Stable ke Growing',       priority:'Prioritas Rendah', color:'#10B981',
    text:`${fmtN(stable)} outlet stagnan. Dorong dengan program insentif atau edukasi layanan baru agar pengelola BUMDes aktif meningkatkan volume.` });
...
/* ════════════════════════════════════════════
   MAIN COMPONENT
════════════════════════════════════════════ */
async function fetchBumdes(bulan) {
  const qs = bulan ? `?bulan=${bulan}` : '';
  const [aRes, oRes] = await Promise.all([
    fetch(`${API}/api/warroom/bumdes/analytics${qs}`, { headers: authHeaders() }),
    fetch(`${API}/api/warroom/bumdes/outlets${qs}`,   { headers: authHeaders() }),
  ]);
  if (!aRes.ok || !oRes.ok) throw new Error('Gagal memuat data BUMDes');
      ...
      <div className="wrfp-page">
        <div className="wrfp-header">
          <div className="wrfp-header-left">
            ...
            <div>
              <div className="wrfp-header-title">WAR-ROOM PA — BUMDES</div>
          ...
          <div className="wrfp-header-badges" style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <span className="wrfp-badge" style={{ background: COLOR2 }}>{bulan ? bulanDisplay(bulan) : 'BUMDes'}</span>
          </div>
        </div>
