...
// class itu memang generik (tabs/panel/kpi/table/modal/pagination), BUKAN
// spesifik OCBC, jadi tidak diduplikasi. Class BARU (prefix wrrm-*) hanya
// ditambahkan utk elemen yang benar-benar baru di halaman ini (bucket waktu,
// validasi saldo, sub-tab Raw Data & Audit) — lihat index.css.
const COLOR = '#003D79';
const ACCENT = '#F4C430';
const TABS = [
...
/* ─── UI atoms (sama dengan pola wrr-* OCBC) ─── */
    ...
    <>
      <div className="wrr-kpi-grid">
        ...
        <KPICard label="Matched Nominal" value={fmtRp(s?.matched_nominal)}
          info="Total nominal FP dari transaksi yang berhasil dicocokkan ke Mandiri." />
        <KPICard label="Pending Bank" value={fmtN(s?.pending_bank_count)} alert={(s?.pending_bank_count || 0) > 0}
          info="Transaksi FP yang belum ditemukan di Mandiri, TAPI masih dalam masa tunggu (grace period, default 30 menit)." />
        <KPICard label="FP Only" value={fmtN(s?.fp_only_count)} alert={(s?.fp_only_count || 0) > 0}
          info="Transaksi FP yang TIDAK ditemukan di Mandiri SETELAH masa tunggu selesai — perlu dicek." />
      ...
      <div className="wrr-panel">
        <div className="wrr-panel-title">
          ...
          <InfoIcon text="Mengecek konsistensi Close Balance antar baris statement: saldo sebelumnya − debit + credit harus sama dengan saldo baris ini. Arah urutan (menaik/menurun) dideteksi otomatis. Ini PERINGATAN level batch, TIDAK mengubah status transaksi individual manapun." />
        </div>
        {!bv ? (
          ...
          <div className="wrrm-balance-row">
            ...
            <span>Arah urutan statement: <strong>{bv.direction === 'ASC' ? 'Menaik (baris pertama = mutasi terlama)' : bv.direction === 'DESC' ? 'Menurun (baris pertama = mutasi terbaru)' : '-'}</strong></span>
          ...
          <table className="wrr-table">
            <thead>
              <tr>
                ...
                <SortableTh label="Principal Mandiri" sortKey="bank_principal" sort={sort} onSort={handleSort} />
                <SortableTh label="Fee Mandiri" sortKey="bank_fee" sort={sort} onSort={handleSort} />
                ...
                <th>Waktu FP</th><th>Post Date Mandiri</th>
                <SortableTh label="Selisih Waktu" sortKey="time_difference_minutes" sort={sort} onSort={handleSort} />
                <th>Account No.</th><th>Outlet</th><th>Produk</th><th>Biller</th>
...
function ResolveModal({ target, onClose, onResolved }) {
  ...
  const handleSubmit = () => {
    ...
    resolveReconciliationMandiri(target.id, { status, notes: notes || undefined })
      .then(() => onResolved())
  ...
  return (
    <Modal title={`Resolve — ${target.id_transaksi || target.reference_no || target.id}`} onClose={onClose}>
      <div className="wrr-detail-grid" style={{ marginBottom: 16 }}>
        <div><span className="wrr-detail-label">ID Transaksi</span><span className="wrr-detail-value">{target.id_transaksi || target.reference_no || '-'}</span></div>
        <div><span className="wrr-detail-label">Account No.</span><span className="wrr-detail-value">{target.account_no || '-'}</span></div>
...
function AuditLogModal({ id, onClose }) {
  ...
  useEffect(() => {
    getReconciliationMandiriLogs(id).then(setLogs).catch(e => setError(e.message)).finally(() => setLoading(false));
  }, [id]);
...
function FeeAnalysisTab({ analytics }) {
  ...
  return (
    <>
      <div className="wrr-kpi-grid">
        <KPICard label="Expected Fee / Transaksi" value={fmtRpFull(fa.expected_fee)}
          info="Nominal fee transfer yang seharusnya dikenakan Mandiri per transaksi (default Rp100, bisa dikonfigurasi per batch)." />
        <KPICard label="Transaksi dengan Fee" value={fmtN(fa.transaction_with_fee_count)}
          info="Jumlah transaksi yang datanya lengkap (principal + fee ditemukan), sehingga fee-nya bisa dihitung." />
        <KPICard label="Actual Fee (Total)" value={fmtRp(fa.actual_fee_total)}
          info="Total fee yang benar-benar terpotong di Mandiri dari seluruh transaksi." />
        <KPICard label="Expected Fee (Total)" value={fmtRp(fa.expected_fee_total)}
          info="Total fee yang SEHARUSNYA terpotong = jumlah transaksi berfee × expected fee." />
        <KPICard label="Fee Variance" value={fmtRp(fa.fee_variance)} alert={Math.abs(fa.fee_variance || 0) > 0}
          info="Selisih Actual Fee Total dikurangi Expected Fee Total." />
        <KPICard label="Transaksi Tanpa Fee" value={fmtN(fa.transaction_without_fee_count)}
          info="Transaksi MATCHED_NO_FEE — principal cocok tapi baris fee tidak ditemukan sama sekali di statement." />
        <KPICard label="Fee Tidak Sesuai" value={fmtN(fa.fee_mismatch_count)} alert={(fa.fee_mismatch_count || 0) > 0}
          info="Jumlah transaksi dengan fee yang TIDAK sama dengan Expected Fee (status FEE_MISMATCH)." />
      </div>
      <div className="wrr-panel">
      ...
      <GroupTable title="Fee per Account Number" rows={fa.by_account_no} />
    </>
  );
}
...
function TimeAnalysisTab({ analytics }) {
  ...
  if (!ta) return <div className="wrr-empty-sub">Belum ada data waktu untuk tanggal ini.</div>;
  const buckets = [
    ...
    { key: 'bucket_gt_30', label: '> 30 menit', tone: 'exception' },
  ];
  return (
    <>
      <div className="wrr-kpi-grid">
        <KPICard label="Rata-rata Selisih Waktu" value={fmtMinutes(ta.avg_minutes)}
          info="Rata-rata selisih (menit) antara time_response FP dan PostDate Mandiri, dari seluruh transaksi yang sudah matched." />
        <KPICard label="Median" value={fmtMinutes(ta.median_minutes)}
          info="Nilai tengah selisih waktu — lebih tahan terhadap outlier dibanding rata-rata." />
        <KPICard label="P95" value={fmtMinutes(ta.p95_minutes)}
          info="95% transaksi memiliki selisih waktu di bawah angka ini." />
        <KPICard label="Maksimum" value={fmtMinutes(ta.max_minutes)}
          info="Selisih waktu terlama yang tercatat pada batch ini." />
      </div>
      <div className="wrr-panel">
        <div className="wrr-panel-title">
          <i className="ti ti-hourglass" style={{ color: COLOR }} /> Distribusi Kategori Keterlambatan
          <InfoIcon text="0-5 menit=normal, 5-15 menit=warning, 15-30 menit=delayed, >30 menit=exception. Ini indikator kecepatan posting bank, BUKAN pembanding nominal — perbedaan waktu tidak membuat status jadi mismatch." />
        </div>
      ...
      <div className="wrr-panel">
        ...
        {(!ta.late_postings || ta.late_postings.length === 0) ? (
          ...
          <div className="wrr-table-wrap">
            <table className="wrr-table">
              <thead><tr><th>ID Transaksi</th><th>Waktu FP</th><th>Post Date Mandiri</th><th>Selisih Waktu</th><th>Status</th></tr></thead>
              <tbody>
                {ta.late_postings.map((r, i) => (
                  <tr key={i}>
                    <td>{r.id_transaksi || '-'}</td><td>{fmtDateTime(r.fp_time_response)}</td>
...
function RawFpSubTab({ date }) {
  ...
  useEffect(() => {
    if (!date) return;
    setLoading(true);
    getReconciliationMandiriRawFp({ date, page, limit }).then(res => { setRows(res.rows || []); setTotal(res.meta?.total || 0); }).finally(() => setLoading(false));
  }, [date, page]);
...
function RawBankSubTab({ date }) {
  ...
  useEffect(() => {
    if (!date) return;
    setLoading(true);
    getReconciliationMandiriRawBank({ date, page, limit }).then(res => { setRows(res.rows || []); setTotal(res.meta?.total || 0); }).finally(() => setLoading(false));
  }, [date, page]);

  if (loading) return <div className="wrr-empty-sub">Memuat...</div>;
  if (!rows.length) return <div className="wrr-empty-sub">Belum ada raw data Mandiri.</div>;
  return (<>
    <div className="wrr-table-wrap">
      <table className="wrr-table">
        <thead>
          <tr>
            <th>Row #</th><th>Account No.</th><th>Post Date</th><th>Remarks</th><th>Additional Desc</th>
            <th>Debit</th><th>Credit</th><th>Close Balance</th><th>Extracted ID</th><th>Row Type</th><th>Extraction Method</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>
              <td>{r.source_row_number ?? '-'}</td><td>{r.account_no || '-'}</td><td>{fmtDateTime(r.post_date_time)}</td>
              <td>{r.remarks || '-'}</td><td>{r.additional_desc || '-'}</td>
              <td>{r.debit !== null ? fmtRp(r.debit) : '-'}</td><td>{r.credit !== null ? fmtRp(r.credit) : '-'}</td>
              <td>{r.close_balance !== null ? fmtRp(r.close_balance) : '-'}</td>
  ...
  return (<>
    <div className="wrr-panel">
      ...
      <div className="wrr-dq-note-grid">
        ...
        <div><span className="wrr-dq-note-label">Account No.</span><span className="wrr-dq-note-value">{meta?.account_no || '-'}</span></div>
        ...
        <div><span className="wrr-dq-note-label">Jumlah Baris Mandiri</span><span className="wrr-dq-note-value">{fmtN(meta?.bank_row_count)}</span></div>
    ...
    <div className="wrr-panel">
      ...
      {recentBatches.length > 0 && (
        <div className="wrr-table-wrap">
          <table className="wrr-table">
            <thead><tr><th>Batch</th><th>Tanggal</th><th>Account No.</th><th>Scope</th><th>Baris FP</th><th>Baris Mandiri</th><th>Sync Terakhir</th><th>Status</th></tr></thead>
            <tbody>
...
function ResolutionHistorySubTab({ date }) {
  ...
  useEffect(() => {
    if (!date) return;
    setLoading(true);
    getReconciliationMandiriResolutionHistory({ date }).then(setLogs).finally(() => setLoading(false));
  }, [date]);

  if (loading) return <div className="wrr-empty-sub">Memuat...</div>;
  if (!logs.length) return <div className="wrr-empty-sub">Belum ada riwayat penyelesaian manual untuk tanggal ini. Gunakan tombol "Riwayat" pada baris di Exception Queue untuk melihat riwayat 1 transaksi spesifik.</div>;
  return (
    <div className="wrr-table-wrap">
      <table className="wrr-table">
...
function RawDataTab({ analytics, date, onExport, exporting }) {
  ...
  const subTabs = [
    ...
    { key: 'bank', label: 'Raw DATA Mandiri' },
    ...
    <>
      <div className="wrr-panel">
        ...
        <div className="wrrm-subtabs">
          {subTabs.map(t => (
            <button key={t.key} className={'wrrm-subtab-btn' + (subTab === t.key ? ' wrrm-subtab-btn--active' : '')} onClick={() => setSubTab(t.key)}>
              {t.label}
            </button>
          ))}
        </div>
...
/* ═══════════════════════════════════════════════════════════════════════
   Main
   ═══════════════════════════════════════════════════════════════════════ */
export default function WarRoomReconciliationMandiri() {
  ...
  const loadAnalytics = useCallback((d) => {
    setLoading(true); setError(null);
    getReconciliationMandiriAnalytics(d ? { date: d } : {})
      .then(res => {
        setAnalytics(res);
  ...
  const handleExport = () => {
    setExporting(true);
    exportReconciliationMandiriCsv({ date: date || undefined })
      .then(blob => downloadBlob(blob, `reconciliation-mandiri-${date || 'export'}.csv`))
      ...
      <div className="wrr-page">
        <div className="wrr-header">
          <div className="wrr-header-left">
            ...
            <div>
              <div className="wrr-header-title">Rekonsiliasi Mandiri</div>
              <div className="wrr-header-sub">Rekonsiliasi transaksi FP terhadap mutasi Bank Mandiri — ekstraksi ID dari Remarks/AdditionalDesc, fee transfer Rp100, dan exception queue.</div>
            </div>
          </div>
        ...
        {!loading && !error && isEmpty && (
          <div className="wrr-empty">
            ...
            <div>{analytics?.message || 'Data rekonsiliasi Mandiri belum tersedia. Jalankan sync Google Sheet terlebih dahulu.'}</div>
          </div>
        )}
