...
const TIPE_COLORS = ['#EA580C','#0EA5E9','#F59E0B','#EF4444','#8B5CF6','#EC4899','#059669','#14B8A6'];
...
function buildInsight(s, bulan) {
  ...
  const paragraph = [
    `Total ${fmtN(s.total_outlet)} outlet MPNG3 terdaftar, ${fmtN(s.mat)} aktif bertransaksi di ${curr} (activation rate ${matPct}%).`,
    arahTrx && arahRev
    ...
    newReg > 0 ? `${fmtN(newReg)} outlet MPNG3 baru registrasi di ${curr}.` : '',
  ...
  if (churned>0)    recs.push({ icon:'🚨', title:'Recovery Call Outlet Hilang',         priority:'Prioritas Tinggi', color:'#DC2626',
    text:`${fmtN(churned)} outlet MPNG3 aktif di ${prev} tapi tidak ada transaksi di ${curr}. Hubungi pengelola outlet segera agar bisa diaktifkan kembali.` });
  if (declining>0)  recs.push({ icon:'📉', title:'Retensi Outlet Declining',             priority:'Prioritas Tinggi', color:'#EF4444',
    text:`${fmtN(declining)} outlet mengalami penurunan TRX vs ${prev}. Follow-up langsung, identifikasi kendala operasional.` });
  ...
  if (nmat>0)       recs.push({ icon:'🌟', title:'Onboarding Outlet Baru',               priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(nmat)} outlet baru aktif di ${curr}. Lakukan pendampingan terstruktur agar konsisten aktif bulan berikutnya.` });
  if (belumAktif>0) recs.push({ icon:'⚠️', title:'Aktivasi Outlet Belum Bertransaksi',  priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(belumAktif)} outlet terdaftar tapi belum pernah bertransaksi. Kunjungi pengelola dan fasilitasi transaksi pertama.` });
  if (growing>0)    recs.push({ icon:'📈', title:'Pertahankan Momentum Growth',          priority:'Prioritas Rendah', color:'#10B981',
    text:`${fmtN(growing)} outlet sedang tumbuh. Berikan apresiasi dan dorong peningkatan volume transaksi.` });
  if (stable>0)     recs.push({ icon:'📌', title:'Dorong Outlet Stable ke Growing',      priority:'Prioritas Rendah', color:'#10B981',
    text:`${fmtN(stable)} outlet stagnan. Dorong dengan program insentif atau edukasi layanan baru agar pengelola aktif meningkatkan volume.` });
...
function ExecutiveTab({ summary: s, lastSync, outlets, bulan }) {
  ...
  return (
    <div>
      {/* KPI Cards — dirancang cocok 1:1 dengan blok ringkasan sheet sumber */}
            ...
            {paged.map(o => {
              ...
              return (
                <tr key={o.id_outlet} className="writ-tr">
                  ...
                  <td className="writ-td" style={{ fontSize:11 }}>{o.tipe_outlet}</td>
...
function TipeTab({ tipes, bulan }) {
  ...
  const onSort = f => { if(sortF===f) setSortD(d=>d==='asc'?'desc':'asc'); else {setSortF(f);setSortD('desc');} };
  const labels = (tipes||[]).map(t=>t.tipe_outlet||'-');
...
/* ════════════════════════════════════════════
   MAIN COMPONENT
════════════════════════════════════════════ */
async function fetchMpng3(bulan) {
  const qs = bulan ? `?bulan=${bulan}` : '';
  const [aRes, oRes] = await Promise.all([
    fetch(`${API}/api/warroom/mpng3/analytics${qs}`, { headers: authHeaders() }),
    fetch(`${API}/api/warroom/mpng3/outlets${qs}`,   { headers: authHeaders() }),
  ]);
  if (!aRes.ok || !oRes.ok) throw new Error('Gagal memuat data MPNG3');
  const [a, o] = await Promise.all([aRes.json(), oRes.json()]);
  return { analytics: a, outlets: o.outlets || [], bulan_list: a.bulan_list || [] };
}

export default function WarRoomMpng3() {
      ...
      <div className="wrfp-page">
        <div className="wrfp-header">
          <div className="wrfp-header-left">
            ...
            <div>
              <div className="wrfp-header-title">WAR-ROOM PA — MPNG3</div>
              <div className="wrfp-header-meta">
                Report MPNG3, PBB, PKB · {fmtN(outlets.length)} outlet
              </div>
            </div>
          </div>
          <div className="wrfp-header-badges" style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <span className="wrfp-badge" style={{ background: COLOR2 }}>{bulan ? bulanDisplay(bulan) : 'MPNG3'}</span>
          </div>
        </div>
        ...
        {loading && <div className="wrfp-loading"><i className="ti ti-loader-2 wrfp-spin" /> Memuat data MPNG3...</div>}
        ...
        {!loading && !error && outlets.length === 0 && (
          <div className="wrfp-empty">
            ...
            <div>Belum ada data outlet MPNG3</div>
