...
const TIPE_COLORS = ['#9333EA','#0EA5E9','#059669','#F59E0B','#EF4444','#EC4899','#8B5CF6','#14B8A6'];
...
function buildInsight(s, bulan) {
  ...
  const newReg     = Number(s.new_reg)    || 0;
  const trxPrev    = Number(s.total_trx_prev) || 0;
  const revPrev    = Number(s.total_rev_prev) || 1;
  const devTrxPct  = trxPrev > 0 ? ((Number(s.dev_trx_total)/trxPrev)*100).toFixed(1) : null;
  const devRevPct  = trxPrev > 0 ? ((Number(s.dev_rev_total)/revPrev)*100).toFixed(1) : null;
  ...
  const paragraph = [
    `Total ${fmtN(s.total_outlet)} outlet LPD terdaftar, ${fmtN(s.mat)} aktif bertransaksi di ${curr} (activation rate ${matPct}%).`,
    arahTrx && arahRev
    ...
    newReg > 0 ? `${fmtN(newReg)} outlet LPD baru registrasi di ${curr}.` : '',
  ...
  if (churned>0)    recs.push({ icon:'🚨', title:'Recovery Call Outlet Hilang',         priority:'Prioritas Tinggi', color:'#DC2626',
    text:`${fmtN(churned)} outlet LPD aktif di ${prev} tapi tidak ada transaksi di ${curr}. Hubungi pengelola LPD segera agar bisa diaktifkan kembali.` });
  if (declining>0)  recs.push({ icon:'📉', title:'Retensi Outlet Declining',             priority:'Prioritas Tinggi', color:'#EF4444',
    text:`${fmtN(declining)} outlet LPD mengalami penurunan TRX vs ${prev}. Follow-up langsung, identifikasi kendala operasional.` });
  if (anomali>0)    recs.push({ icon:'🔍', title:'Investigasi Anomali Revenue',          priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(anomali)} outlet menunjukkan TRX naik tapi revenue turun. Cek pergeseran ke jenis layanan bernilai rendah.` });
  if (nmat>0)       recs.push({ icon:'🌟', title:'Onboarding Outlet LPD Baru',           priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(nmat)} outlet LPD baru aktif di ${curr}. Lakukan pendampingan terstruktur agar konsisten aktif bulan berikutnya.` });
  if (belumAktif>0) recs.push({ icon:'⚠️', title:'Aktivasi Outlet Belum Bertransaksi',  priority:'Prioritas Sedang', color:'#F59E0B',
    text:`${fmtN(belumAktif)} outlet LPD terdaftar tapi belum pernah bertransaksi. Kunjungi pengelola dan fasilitasi transaksi pertama.` });
  if (growing>0)    recs.push({ icon:'📈', title:'Pertahankan Momentum Growth',          priority:'Prioritas Rendah', color:'#10B981',
    text:`${fmtN(growing)} outlet LPD sedang tumbuh. Berikan apresiasi dan dorong peningkatan volume layanan transaksi.` });
  if (stable>0)     recs.push({ icon:'📌', title:'Dorong Outlet Stable ke Growing',      priority:'Prioritas Rendah', color:'#10B981',
    text:`${fmtN(stable)} outlet stagnan. Dorong dengan program insentif atau edukasi layanan baru agar pengelola LPD aktif meningkatkan volume.` });
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:14 }}>
        ...
        <KPICard icon="ti-user-plus"    label={`New Reg ${curr}`}          value={fmtN(s.new_reg)}         sub={`Outlet baru daftar ${curr}`} accent="#0EA5E9" />
        <KPICard icon="ti-device-desktop" label="EDC"                      value={fmtN(s.edc)}             sub="Outlet tipe EDC" accent="#EC4899" />
...
/* ════════════════════════════════════════════
   MAIN COMPONENT
════════════════════════════════════════════ */
async function fetchPaLpd(bulan) {
  const qs = bulan ? `?bulan=${bulan}` : '';
  const [aRes, oRes] = await Promise.all([
    fetch(`${API}/api/warroom/pa-lpd/analytics${qs}`, { headers: authHeaders() }),
    fetch(`${API}/api/warroom/pa-lpd/outlets${qs}`,   { headers: authHeaders() }),
  ]);
  if (!aRes.ok || !oRes.ok) throw new Error('Gagal memuat data PA LPD');
  const [a, o] = await Promise.all([aRes.json(), oRes.json()]);
  return { analytics: a, outlets: o.outlets || [], bulan_list: a.bulan_list || [] };
}

export default function WarRoomPaLpd() {
      ...
      <div className="wrfp-page">
        <div className="wrfp-header">
          <div className="wrfp-header-left">
            <i className="ti ti-building-bank" style={{ color: COLOR, fontSize: 22 }} />
            <div>
              <div className="wrfp-header-title">WAR-ROOM PA — LPD</div>
              <div className="wrfp-header-meta">
                Monitoring Outlet Lembaga Perkreditan Desa · {fmtN(outlets.length)} outlet
              </div>
            </div>
          </div>
          <div className="wrfp-header-badges" style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <span className="wrfp-badge" style={{ background: COLOR2 }}>{bulan ? bulanDisplay(bulan) : 'LPD'}</span>
          </div>
        </div>
        ...
        {loading && <div className="wrfp-loading"><i className="ti ti-loader-2 wrfp-spin" /> Memuat data PA LPD...</div>}
        ...
        {!loading && !error && outlets.length === 0 && (
          <div className="wrfp-empty">
            <i className="ti ti-building-bank" style={{ color: COLOR }} />
            <div>Belum ada data outlet LPD</div>
