...
import { getFarmingAnalytics } from '../services/api';
...
function OutletDetailTab({ data }) {
  ...
  const filtered = useMemo(() => {
    let r = allOutlets;
    if (search)               r = r.filter(o=>o.id_outlet.toLowerCase().includes(search.toLowerCase()));
    if (filterStatus!=='all') r = r.filter(o=>o.status===filterStatus);
    const dir = sortDir==='asc'?1:-1;
    r = [...r].sort((a,b)=>dir*((Number(a[sortBy])||0)-(Number(b[sortBy])||0)));
    return r;
    ...
    <div className="wrfp-chart-card">
      <div className="wrfp-filter-bar">
        <div className="wrfp-search-wrap">
          ...
          <input className="wrfp-search" placeholder="Cari ID Outlet…" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} />
        </div>
        <select className="wrfp-select" value={filterStatus} onChange={e=>{setFilterStatus(e.target.value);setPage(1);}}>
      ...
      <div className="wrfp-table-wrap">
        <table className="wrfp-table">
          ...
          <tbody>
            ...
            {!displayed.length && <tr><td colSpan={8} style={{textAlign:'center',padding:'20px',color:'#9CA3AF'}}>Tidak ada data</td></tr>}
          </tbody>
        </table>
      </div>
...
function RevenueAnalysisTab({ data }) {
  ...
  const scatterGroups = useMemo(() => {
    ...
    for (const o of (all_outlets||[])) {
      if (!Number(o.trx_jun_period)) continue;
