...
import { getLpdAnalytics, getLpdOutlets } from '../services/api';
...
function buildInsight(s) {
  ...
  const paragraph = [
    `Total ${fmtN(s.total_outlet)} outlet LPD terdaftar, ${fmtN(s.mat)} aktif bertransaksi di Juni (activation rate ${matPct}%).`,
    arahTrx && arahRev
  ...
  if (churned > 0)   recs.push({ icon: '🚨', title: 'Recovery Call Outlet Hilang',         priority: 'Prioritas Tinggi',  color: '#DC2626',
    text: `${fmtN(churned)} outlet LPD aktif di Mei tapi tidak ada transaksi di Juni. Hubungi pengelola LPD segera agar bisa diaktifkan kembali sebelum akhir bulan.` });
  if (declining > 0) recs.push({ icon: '📉', title: 'Retensi Outlet Declining',             priority: 'Prioritas Tinggi',  color: '#EF4444',
    text: `${fmtN(declining)} outlet LPD mengalami penurunan TRX vs Mei. Follow-up langsung, identifikasi kendala operasional pengelola, dan berikan dukungan teknis.` });
  if (anomali > 0)   recs.push({ icon: '🔍', title: 'Investigasi Anomali Revenue',          priority: 'Prioritas Sedang',  color: '#F59E0B',
    text: `${fmtN(anomali)} outlet menunjukkan TRX naik tapi revenue turun. Cek apakah ada pergeseran ke jenis layanan bernilai rendah atau ada penyesuaian tarif LPD.` });
  if (nmat > 0)      recs.push({ icon: '🌟', title: 'Onboarding Outlet LPD Baru',           priority: 'Prioritas Sedang',  color: '#F59E0B',
    text: `${fmtN(nmat)} outlet LPD baru aktif bulan ini. Lakukan pendampingan terstruktur kepada pengelola dan kunjungan rutin agar konsisten aktif di bulan berikutnya.` });
  if (belumAktif > 0) recs.push({ icon: '⚠️', title: 'Aktivasi Outlet Belum Bertransaksi', priority: 'Prioritas Sedang',  color: '#F59E0B',
    text: `${fmtN(belumAktif)} outlet LPD terdaftar tapi belum pernah bertransaksi. Kunjungi pengelola, edukasi cara menggunakan layanan, dan fasilitasi transaksi pertama.` });
  if (growing > 0)   recs.push({ icon: '📈', title: 'Pertahankan Momentum Growth',          priority: 'Prioritas Rendah',  color: '#10B981',
    text: `${fmtN(growing)} outlet LPD sedang tumbuh. Berikan apresiasi kepada pengelola, lakukan komunikasi rutin, dan dorong peningkatan volume layanan transaksi.` });
  if (stable > 0)    recs.push({ icon: '📌', title: 'Dorong Outlet Stable ke Growing',      priority: 'Prioritas Rendah',  color: '#10B981',
    text: `${fmtN(stable)} outlet stagnan (TRX tidak berubah). Dorong dengan program insentif atau edukasi layanan baru agar pengelola LPD aktif meningkatkan volume.` });
...
/* ════════════════════════════════════════════
   MAIN COMPONENT
════════════════════════════════════════════ */
export default function WarRoomLpd() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [a, o] = await Promise.all([getLpdAnalytics(), getLpdOutlets()]);
        setAnalytics(a);
      ...
      <div className="wrfp-page">
        <div className="wrfp-header">
          <div className="wrfp-header-left">
            ...
            <div>
              <div className="wrfp-header-title">WAR-ROOM TERRITORY LPD</div>
          ...
          <div className="wrfp-header-badges">
            <span className="wrfp-badge" style={{ background: COLOR }}>LPD</span>
        ...
        {loading && <div className="wrfp-loading"><i className="ti ti-loader-2 wrfp-spin" /> Memuat data LPD...</div>}
