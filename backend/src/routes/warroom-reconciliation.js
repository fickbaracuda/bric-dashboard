...
// ─────────────────────────────────────────────────────────────────────────
// GET /api/warroom/reconciliation/ocbc/balance-needs-periodic
// Tab "Kebutuhan Saldo" — kebutuhan saldo OCBC per jam/tanggal utk suatu
// PERIODE (bukan 1 hari, beda dari 5 tab lain di halaman ini). READ-ONLY
// murni, TIDAK menyentuh matching engine/sync/coverage/Exception Queue.
//
// Sumber "transaksi FP" = recon_results.id_transaksi IS NOT NULL (SAMA
// definisi dgn `fpResultRows` di dailyReportHandler) -- recon_results sudah
// deduped 1 baris per id_transaksi oleh engine (processedIds Set), dan
// BANK_ONLY/REVERSAL-tanpa-FP sintetis (id_transaksi NULL) otomatis
// terkecuali, persis sesuai spec "BANK_ONLY tidak masuk kebutuhan saldo FP".
//
// Expected fee TIDAK di-hardcode Rp25 -- diturunkan PER BATCH dari data yang
// SUDAH ADA (bank_fee - variance_fee pada baris matched batch itu, lihat
// reconcileTransactionsWithCoverage baris ~655/856), fallback DEFAULT_FEE_BIFAST
// hanya kalau batch itu genuinely tidak punya baris matched sama sekali
...
module.exports = {
  ...
  // exported utk unit test (pure, tidak menyentuh DB) — tab "Kebutuhan Saldo"
  computeOcbcBalanceNeedsPeriodic,
  dateRangeArray,
