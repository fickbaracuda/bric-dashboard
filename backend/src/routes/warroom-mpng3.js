...
//
// Domain BARU, terpisah dari war-room lain. Arsitektur PERSIS mengikuti
// pola ASDP/LPD/BUMDes versi Payment Agent (multi-bulan, sheet-per-bulan
// bertambah tiap bulan, key (bulan, id_outlet), chunked upsert) — lihat
// backend/src/routes/warroom-pa-lpd.js sebagai referensi.
const pool = require('../db');

const SYNC_TOKEN = process.env.APPS_SCRIPT_TOKEN || 'bric2026bimasaktisecret';

const CHUNK = 500; // 500 x 15 params = 7500, aman < 65535
...
async function upsertChunk(bulan, rows) {
  ...
  await pool.query(`
    INSERT INTO warroom_mpng3_outlet
...
async function syncHandler(req, res) {
  ...
  setImmediate(async () => {
    try {
      ...
      console.log(`[mpng3 sync] done: ${valid.length} outlets, bulan ${bulan}`);
    } catch (err) {
      console.error(`[mpng3 sync] error bulan ${bulan}:`, err.message);
    }
  });
}
...
async function analyticsHandler(req, res) {
  try {
    const blRes = await pool.query(
      `SELECT DISTINCT bulan FROM warroom_mpng3_outlet ORDER BY bulan DESC`
    );
    ...
    const [sumR, upR, kotaR, tipeR, syncR] = await Promise.all([
      // Ringkasan KPI ini SENGAJA dirancang cocok 1:1 dengan blok ringkasan
      // di baris 1-11 sheet sumber (Transaksi, Revenue trx, Trx New MAT,
      // Rev New MAT, MAT, NMAT, NMAT min 100 trx, MAT Min 300 trx, Dev Trx,
      // Dev Rev) — berguna sebagai cross-check terhadap angka sheet.
      pool.query(`
        SELECT
        ...
        FROM warroom_mpng3_outlet WHERE bulan=$1
      `, [bulan]),
      ...
      pool.query(
        'SELECT MAX(synced_at) AS last_sync, COUNT(*) AS total FROM warroom_mpng3_outlet WHERE bulan=$1',
        [bulan]
      ),
    ]);
    ...
    console.error('mpng3 analytics:', err);
...
async function outletsHandler(req, res) {
  try {
    ...
    console.error('mpng3 outlets:', err);
    res.status(500).json({ error: err.message });
  }
}

module.exports = { syncHandler, analyticsHandler, outletsHandler, _internal: { toIsoDate } };
