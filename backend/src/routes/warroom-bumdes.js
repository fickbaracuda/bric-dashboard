...
async function syncHandler(req, res) {
  ...
  // Deduplicate by id_outlet — Google Sheet bisa punya duplikat dalam 1 sheet
  const seen = new Map();
  ...
  setImmediate(async () => {
    try {
      ...
      console.log(`[bumdes sync] done: ${valid.length} outlets, bulan ${bulan}`);
    } catch (err) {
      console.error(`[bumdes sync] error bulan ${bulan}:`, err.message);
    }
  });
}
...
async function analyticsHandler(req, res) {
  try {
    const blRes = await pool.query(
      `SELECT DISTINCT bulan FROM warroom_bumdes_outlet ORDER BY bulan DESC`
    );
    ...
    const [sumR, upR, kotaR, tipeR, syncR] = await Promise.all([
      pool.query(`
        ...
        FROM warroom_bumdes_outlet WHERE bulan=$1
      `, [bulan]),
      ...
      pool.query(
        'SELECT MAX(synced_at) AS last_sync, COUNT(*) AS total FROM warroom_bumdes_outlet WHERE bulan=$1',
        [bulan]
      ),
    ]);
